...
# Recruiter-branded microsites may advertise an unnamed client. Treating the
# recruiter brand as the employer would poison A company learning, so those
# pages stay unresolved unless the actual employer is stated in vacancy text.
...
def _company_from_vacancy_text(text: str) -> str:
    """Extract employer from Jobs.cz/Prace.cz vacancy-specific labels."""
    patterns = (
        # Standard Jobs.cz detail and the email-dialog copy both expose the
        # actual employer even when generic navigation confused the fallback.
...
def recover_czech_board_company(source_id: str, html: str, current_company: object = "") -> str:
    """Recover a real employer identity without guessing from navigation text.

    Existing valid JSON-LD employer data always wins. Recovery is only attempted
    for Jobs.cz / Prace.cz when the current value is blank or a known artefact.
    """
    ...
    # The ČEZ career microsite deliberately brands itself as "...kde jinde."
    # rather than with the employer name, but its page chrome identifies the
    # Skupina ČEZ careers site unambiguously.
