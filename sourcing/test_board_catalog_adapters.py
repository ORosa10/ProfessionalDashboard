...
from sourcing.board_catalog_adapters import CONFIG, _relevant, extract_catalog_links, extract_detail_fields
...
class CatalogAdapterTest(unittest.TestCase):
    def test_startupjobs(self):
        ...
        self.assertEqual(extract_catalog_links("startupjobs-cz", html, 5), ["https://www.startupjobs.cz/nabidka/105017/finance-controller-part-time"])

    def test_jobwinner(self):
        html = '<a href="/job/14645177">Konzern-Treasury</a>'
        self.assertEqual(extract_catalog_links("jobwinner-ch", html, 5), ["https://www.jobwinner.ch/job/14645177"])
    ...
    def test_nzz_only_extracts_real_vacancy_details(self):
        ...
        '''
        self.assertEqual(extract_catalog_links("nzz-jobs-ch", html, 5), ["https://jobs.nzz.ch/job/treasury-manager-zuerich/496136"])

    def test_jobserve_uses_nationwide_finance_catalogue(self):
        listing = CONFIG["jobserve-uk"]["listing"]
        self.assertIn("Finance%2C-Accounting%2BBanking-sector-jobs-in-United-Kingdom", listing)

    def test_jobserve_requires_finance_signal_in_title(self):
        self.assertFalse(_relevant("Senior Data Architect", "Banking client with financial risk and investment systems.", "jobserve-uk"))
    ...
    def test_jobbank(self):
        ...
        self.assertEqual(extract_catalog_links("jobbank-dk", html, 5), ["https://jobbank.dk/job/3067629/"])

    def test_jobunivers(self):
        html = '<a href="/job/finans-oekonomi-og-regnskab/?job=7746&offset=0">Finance Manager</a>'
        self.assertEqual(extract_catalog_links("jobunivers-dk", html, 5), ["https://www.jobunivers.dk/job/finans-oekonomi-og-regnskab/?job=7746&offset=0"])

    def test_ledigajobb(self):
        html = '<a href="/jobb/c348d8/example">Treasury Analyst</a>'
        self.assertEqual(extract_catalog_links("ledigajobb-se", html, 5), ["https://ledigajobb.se/jobb/c348d8/example"])
